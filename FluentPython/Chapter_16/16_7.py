...
def averager():
    ...
    return Result(
        count, average
    )  # когда цикл закончится (значение term будет равно None) генератор вернёт результат
...
def report(results: {str: Result}):
    ...
    for key, value in sorted(results.items()):  # сортирует ключ и значения
        ...
        print(
            "{:2} {:5} average {:.2f}{}".format(  # форматирование строки
...
def frange(start, stop, step):
    ...
    return itertools.takewhile(
        lambda x: x < stop, itertools.count(start, step)
    )  # создаёт генератор float
