...
class Vector2d:
    ...
    def frombytes(cls, octets):
        ...
        memv = memoryview(octets[1:]).cast(
            typecode
        )  # создаем memoryview для двоичной последовательности
        return cls(*memv)
    ...
    def __repr__(self):
        ...
        return "{} ({!r}, {!r})".format(
            class_name, *self
        )  # метод строит строку представления, итерируя конструктор
    ...
    def __bytes__(self):
        return bytes(
            [ord(self.typecode)]
        ) + bytes(  # преобразуем typecode в bytes и конкатенируем
    ...
    def __abs__(self):
        return math.hypot(
            self.x, self.y
        )  # преобразуем в абсолютное значение и любое число отличное от нуля
        # вернет True
