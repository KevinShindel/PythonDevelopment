...
import asyncio  # модуль aiohttp нужен для HTTP
...
class AioHttpParser(BaseParser):
    ...
    async def get_flag(self, cc: str):  # повторно используем метод get_flag
        ...
        async with aiohttp.request(
            "GET", url
        ) as response:  # блокирующие операции реализованы в виде сопрограмм
            return await response.read()

    async def download_one(self, cc):
        image = await self.get_flag(
            cc
    ...
    def download_many(self, cc_list):
        ...
        to_do = [
            self.download_one(cc) for cc in sorted(cc_list)
        ]  # строим цикл генератор для загрузки обьектов
        wait_coro = asyncio.wait(
            to_do
        )  # неблокирующая функция, которая завершается по окончанию всех сопрограмм
        res, _ = loop.run_until_complete(
            wait_coro
        )  # выполняем цикл обработки событий пока сопрограмма wait_coro не завершится
