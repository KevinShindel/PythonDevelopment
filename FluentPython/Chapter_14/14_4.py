...
def main():
    gen = itertools.takewhile(
        lambda n: n < 3, itertools.count(1, 0.5)  # фунция останавливается по условию
    ...
    r = list(
        itertools.takewhile(vowel, "Aardvark")
    ...
    )  # отдаёт все элементы со среза, операция ленивая
    print(r)  # ['A', 'a', 'r', 'd']
    ...
    )  # декартово произведение (комбинаторика)
    # print(rez)

    rez = list(
        zip(range(10), sorted(range(10), reverse=True))
    )  # связывает итерируемые элементы в кортежи
    print(rez)

    rez = list(
        itertools.zip_longest(range(5), range(10), fillvalue=0)
    )  # связывает неравнодлинные итерируемые списки
    print(rez)
