        ...
        with ThreadPoolExecutor(max_workers=self.WORKERS) as executor:
            ...
            for cc in sorted(cc_list):
                future = executor.submit(
                    self.download_one, cc
                )  # планирует выполенние одного вызываемого обьекта, первый аргумент сам обьекта остальные параметры
                to_do_map[future] = cc
            done_iter = as_completed(
                to_do_map
            )  # возвращает итератор который отдаёт будущие обьекты по мере их выполнения
