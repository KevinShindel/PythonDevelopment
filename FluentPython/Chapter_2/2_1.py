...
def sorted_list():
    ...
    )  # сортировка в алфавитном порядке задом на перед
...
def list_vs_array_compersation():
    floats = array(
        "d", (random.random() for _ in range(10**7))
    )  # массив из милиона чисел
...
def deque_principal():
    ...
    dq = [
        dq.append(random.random()) for _ in range(10**6)
    ]  # в цикле добавляем еще миллион
    end = perf_counter()
    ...
    def_list = [
        def_list.append(random.random()) for _ in range(10**6)
    ]  # в цикле добавляем еще миллион
    end = perf_counter()
    total = round(end - start, 2)
    print(f"done at {total} seconds")
