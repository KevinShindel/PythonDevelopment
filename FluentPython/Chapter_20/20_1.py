...
class Quantity:  # дескриптор основан на протоколе, поэтому доплнительное наследование не требуется
    ...
    def __init__(self, storage_name):
        self.storage_name = (
            storage_name  # в каждом экземляре имеется поле для хранения значения
        )

    def __set__(
        self, instance, value
    ):  # метод set вызывается при любой попытке присвоить значение
        if value > 0:
            instance.__dict__[self.storage_name] = (
                value  # необходимо работать с атрибутом __dict__
            )
        else:
